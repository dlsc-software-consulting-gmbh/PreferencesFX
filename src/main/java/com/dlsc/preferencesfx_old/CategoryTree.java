...
public class CategoryTree extends TreeView {
  ...
  private Predicate<Category> filterPredicate = category -> {
    ...
    boolean categoryMatch = StringUtils.containsIgnoreCase(category.getDescription(), searchText.get());
    ...
    if (category.getGroups() != null) {
      ...
      settingMatch = category.getGroups().stream()
          ...
          .anyMatch(setting -> StringUtils.containsIgnoreCase(setting.getDescription(), searchText.get()));
      // look in groups too
      groupMatch = category.getGroups().stream()
          .anyMatch(group -> StringUtils.containsIgnoreCase(group.getDescription(), searchText.get()));
    }
